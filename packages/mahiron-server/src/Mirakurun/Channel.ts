...
export default class Channel {
  ...
  private _epgGatheringInterval: number =
    _.config.server.epgGatheringInterval || 1000 * 60 * 30 // 30 mins
  ...
  private _load(): void {
    ...
    channels.forEach((channel, i) => {
      if (typeof channel.name !== 'string') {
        log.error(
          'invalid type of property `name` in channel#%d configuration',
          i
        )
        return
      }
      ...
      if (
        ...
        log.error(
          'invalid type of property `type` in channel#%d (%s) configuration',
          i,
          channel.name
        )
        return
      ...
      if (typeof channel.channel !== 'string') {
        log.error(
          'invalid type of property `channel` in channel#%d (%s) configuration',
          i,
          channel.name
        )
        return
      ...
      if (channel.satelite && !channel.satellite) {
        log.warn(
          'renaming deprecated property name `satelite` to `satellite` in channel#%d (%s) configuration',
          i,
          channel.name
        )
      ...
      if (channel.satellite && typeof channel.satellite !== 'string') {
        log.error(
          'invalid type of property `satellite` in channel#%d (%s) configuration',
          i,
          channel.name
        )
        return
      ...
      if (channel.space && typeof channel.space !== 'number') {
        log.error(
          'invalid type of property `space` in channel#%d (%s) configuration',
          i,
          channel.name
        )
        return
      ...
      if (channel.freq !== undefined && typeof channel.freq !== 'number') {
        log.error(
          'invalid type of property `freq` in channel#%d (%s) configuration',
          i,
          channel.name
        )
        return
      ...
      if (
        ...
        log.error(
          'invalid type of property `polarity` in channel#%d (%s) configuration',
          i,
          channel.name
        )
        return
      ...
      if (channel.serviceId && typeof channel.serviceId !== 'number') {
        log.error(
          'invalid type of property `serviceId` in channel#%d (%s) configuration',
          i,
          channel.name
        )
        return
      ...
      if (channel.tsmfRelTs && typeof channel.tsmfRelTs !== 'number') {
        log.error(
          'invalid type of property `tsmfRelTs` in channel#%d (%s) configuration',
          i,
          channel.name
        )
        return
  ...
  private _epgGatherer(): void {
    queue.add(async () => {
      const networkIds = [
        ...new Set(_.service.items.map((item) => item.networkId)),
      ]
        ...
        queue.add(async () => {
          if (service.epgReady === true) {
            ...
            if (now - service.epgUpdatedAt < this._epgGatheringInterval) {
              log.info(
                'Network#%d EPG gathering has skipped by `epgGatheringInterval`',
                networkId
              )
              return
            }
            if (now - service.epgUpdatedAt > 1000 * 60 * 60 * 6) {
              // 6 hours
              log.info(
                'Network#%d EPG gathering is resuming forcibly because reached maximum pause time',
                networkId
              )
              service.epgReady = false
            } else {
              const currentPrograms = _.program
                .findByNetworkIdAndTime(networkId, now)
                .filter(
                  (program) => !!program.name && program.name !== '放送休止'
                )
              if (currentPrograms.length === 0) {
                const networkPrograms = _.program.findByNetworkId(networkId)
                if (networkPrograms.length > 0) {
                  log.info(
                    'Network#%d EPG gathering has skipped because broadcast is off',
                    networkId
                  )
                  return
                }
          ...
          if (status.epg[networkId] === true) {
            log.info(
              'Network#%d EPG gathering is already in progress on another stream',
              networkId
            )
            return
          }
