...
export default class EPG {
  private _epg: {
    [networkId: number]: {
      [serviceId: number]: { [eventId: number]: EventState }
    }
  } = {}
  ...
  write(eit: EIT) {
    ...
    for (const e of eit.events) {
      ...
      if (!service[e.event_id]) {
        ...
        if (
          (!state.present || (state.present && isP)) &&
          isOutOfDate(eit, state.version)
        ) {
        ...
        switch (d.descriptor_tag) {
            ...
            if (!state.extended._descs[d.descriptor_number]) {
              ...
              for (const descs of state.extended._descs) {
                for (const desc of descs) {
                  const key =
                    desc.item_description_length === 0
                      ? current
          ...
          case 0x50:
            ...
            _.program.set(state.programId, {
              video: {
                type:
                  <db.ProgramVideoType>STREAM_CONTENT[d.stream_content] || null,
                resolution:
                  <db.ProgramVideoResolution>COMPONENT_TYPE[d.component_type] ||
                  null,
          ...
          case 0xc4:
            {
              ...
              state.audio.version[eit.table_id][d.component_tag] =
                eit.version_number
          ...
          case 0xd5:
            ...
            _.program.set(state.programId, {
              series: {
                ...
                expiresAt:
                  d.expire_date_valid_flag === 1
                    ? getTimeFromMJD(
                        Buffer.from(d.expire_date.toString(16), 'hex')
                      )
                    : -1,
          ...
          case 0xd6:
            ...
            state.group._groups[d.group_type] =
              d.group_type < 4
...
function isOutOfDateLv2(
  eit: EIT,
  versionRecord: VersionRecord<VersionRecord>,
  lv2: number
): boolean {
  if (
    (versionRecord[0x4e] !== undefined && eit.table_id !== 0x4e) ||
...
function getRelatedProgramItem(event: any): db.ProgramRelatedItem {
  return {
    type:
      this.group_type === 1
        ? 'shared'
        : this.group_type === 2 || this.group_type === 4
        ? 'relay'
        : 'movement',
