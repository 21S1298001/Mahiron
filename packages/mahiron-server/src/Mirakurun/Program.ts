...
export function getProgramItemId(
  networkId: number,
  serviceId: number,
  eventId: number
): number {
  return parseInt(
    `${networkId}${serviceId.toString(10).padStart(5, '0')}${eventId
      .toString(10)
      .padStart(5, '0')}`,
    10
  )
...
export default class Program {
  ...
  private _programGCInterval =
    _.config.server.programGCInterval || 1000 * 60 * 60 // 1 hour
  ...
  add(item: db.Program, firstAdd = false): void {
  ...
  findByNetworkIdAndTime(networkId: number, time: number): db.Program[] {
    ...
    for (const item of this._itemMap.values()) {
      if (
        ...
        item.startAt <= time &&
        item.startAt + item.duration > time
      ) {
        items.push(item)
      }
    }
  ...
  private _gc(): void {
    ...
    queue.add(async () => {
      ...
      for (const item of this._itemMap.values()) {
        if (
          (item.duration === 1 ? longExp : shortExp) >
