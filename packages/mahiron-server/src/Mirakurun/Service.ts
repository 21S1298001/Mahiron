...
export default class Service {
  ...
  static async getLogoDataMTime(
    ...
    try {
      return (await fsPromises.stat(Service.getLogoDataPath(networkId, logoId)))
        .mtimeMs
    } catch (e) {
      return 0
    }
  ...
  static async isLogoDataExists(
    ...
    try {
      return (
        await fsPromises.stat(Service.getLogoDataPath(networkId, logoId))
      ).isFile()
    } catch (e) {
      return false
    }
    ...
    for (const service of services) {
      ...
      if (service.logoData) {
        ...
        services
          .filter(
            (s) =>
              s.networkId === service.networkId && s.logoId === service.logoId
          )
          .forEach((s) => {
            delete s.logoData
          })
