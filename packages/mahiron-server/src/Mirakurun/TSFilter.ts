...
export default class TSFilter extends EventEmitter {
  ...
  write(chunk: Buffer): void {
    ...
    if (this._buffer.length !== 0) {
      if (
        ...
        const head =
          this._buffer.length - this._maxBufferBytesBeforeReady / PACKET_SIZE
        if (head > 0) {
  ...
  private _processPackets(packets: Buffer[]): void {
    ...
    for (let packet of packets) {
      ...
      if (this._tsmfEnableTsmfSplit) {
        ...
        if (
          this._tsmfRelativeStreamNumber[this._tsmfSlotCounter - 1] !==
          this._tsmfTsNumber
        ) {
          continue
        }
  ...
  private _onSDT(pid: number, data: any): void {
    ...
    for (const service of data.services) {
      ...
      if (
        _services.some((_service) => _service.id === service.service_id) ===
        false
      ) {
        _services.push({
  ...
  private async _standbyLogoData(): Promise<void> {
    ...
    for (const networkId in logoIdNetworkMap) {
      for (const logoId of logoIdNetworkMap[networkId]) {
        ...
        if (
          now -
            (await Service.getLogoDataMTime(this._targetNetworkId, logoId)) >
          logoDataInterval
        ) {
          if (this._closed) {
  ...
  private _updateEpgState(data: any): void {
    ...
    const targetFlags =
      (data.table_id & 0x0f) < 0x08 ? stateBySrv.basic : stateBySrv.extended
    ...
    if (flagsId === 0 && this._streamTime !== null) {
      const segment =
        ((this._streamTime + 9 * 60 * 60 * 1000) / (3 * 60 * 60 * 1000)) & 0x07
