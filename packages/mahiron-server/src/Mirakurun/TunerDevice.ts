...
export default class TunerDevice extends EventEmitter {
  ...
  async startStream(
    ...
    log.debug(
      'TunerDevice#%d start stream for user `%s` (priority=%d)...',
      this._index,
      user.id,
      user.priority
    )
    ...
    if (this._isAvailable === false) {
      throw new Error(
        util.format('TunerDevice#%d is not available', this._index)
      )
    }
    ...
    if (channel) {
      if (this._config.types.includes(channel.type) === false) {
        throw new Error(
          util.format(
            'TunerDevice#%d is not supported for channel type `%s`',
            this._index,
            channel.type
          )
        )
      ...
      if (this._stream) {
        if (channel.channel !== this._channel.channel) {
          if (user.priority <= this.getPriority()) {
            throw new Error(
              util.format(
                'TunerDevice#%d has higher priority user',
                this._index
              )
            )
          }
    ...
    log.info(
      'TunerDevice#%d streaming to user `%s` (priority=%d)',
      this._index,
      user.id,
      user.priority
    )
  ...
  endStream(user: User): void {
    log.debug(
      'TunerDevice#%d end stream for user `%s` (priority=%d)...',
      this._index,
      user.id,
      user.priority
    )
    ...
    log.info(
      'TunerDevice#%d end streaming to user `%s` (priority=%d)',
      this._index,
      user.id,
      user.priority
    )
  ...
  async getRemotePrograms(query?: ProgramsQuery): Promise<apid.Program[]> {
    if (!this._isRemote) {
      throw new Error(
        util.format('TunerDevice#%d is not remote device', this._index)
      )
    }
    ...
    log.info(
      'TunerDevice#%d fetched %d remote programs',
      this._index,
      programs.length
    )
  ...
  private _spawn(ch: ChannelItem): void {
    ...
    if (this._config.dvbDevicePath) {
      ...
      cat.once('error', (err) => {
        log.error(
          'TunerDevice#%d cat process error `%s` (pid=%d)',
          this._index,
          err.name,
          cat.pid
        )
    ...
    this._process.once('error', (err) => {
      log.fatal(
        'TunerDevice#%d process error `%s` (pid=%d)',
        this._index,
        err.name,
        this._process.pid
      )
      ...
      if (this._fatalCount >= 3) {
        log.fatal(
          'TunerDevice#%d has something fault! **RESTART REQUIRED** after fix it.',
          this._index
        )
      ...
      setTimeout(
        ...
        this._config.dvbDevicePath ? 1000 : 100
      )
    })
    ...
    this._updated()
    log.info(
      'TunerDevice#%d process has spawned by command `%s` (pid=%d)',
      this._index,
      cmd,
      this._process.pid
    )
  ...
  private async _kill(close: boolean): Promise<void> {
    ...
    if (!this._process || !this._process.pid) {
      throw new Error(
        util.format('TunerDevice#%d has not process', this._index)
      )
    ...
    await new Promise<void>((resolve) => {
      ...
      if (process.platform === 'win32') {
        ...
        const timer = setTimeout(() => {
          log.warn(
            'TunerDevice#%d will force killed because SIGTERM timed out...',
            this._index
          )
          this._process.kill('SIGKILL')
        }, 6000)
  ...
  private _release(): void {
    ...
    if (this._closing === false && this._users.size !== 0) {
      log.warn(
        'TunerDevice#%d respawning because request has not closed',
        this._index
      )
