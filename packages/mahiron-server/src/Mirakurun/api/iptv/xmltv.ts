...
function getDateTime(time: number): string {
  return (
    new Date(time).toISOString().replace(/\..+$/, '').replace(/[-:T]/g, '') +
    ' +0000'
  )
}
...
function getGenreStrings(genres: Program['genres']) {
  ...
  for (const genre of genres) {
    if (genre.lv1 === 14) {
      ...
      // 標準
      stringSet.add(
        `${GENRE_LV1[genre.lv1]} - ${GENRE_LV2[genre.lv1 * 0x10 + genre.lv2]}`
      )
    }
  }
...
export const get: Operation = async (req, res) => {
  ...
  for (const program of _.program.itemMap.values()) {
    ...
    x += `<programme start="${getDateTime(
      program.startAt
    )}" stop="${getDateTime(program.startAt + program.duration)}" channel="${
      service.id
    }">\n`
