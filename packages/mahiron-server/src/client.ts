...
const spec = yaml.load(
  fs.readFileSync(__dirname + '/../api.yml', 'utf8')
...
export default class Client {
  ...
  ): Promise<Response> | Promise<ErrorResponse> {
  ...
  async call(
    ...
    option: RequestOption = {}
  ): Promise<any | http.IncomingMessage> {
    if (!this._docs) {
      await this._getDocs()
    }
  ...
  async getServiceStreamByChannel(opt: {
    type: apid.ChannelType
    channel: string
    sid: apid.ServiceId
    decode?: boolean
      ...
      const req = http.request(opt, (res) => {
        if (
          ...
          if (/^\//.test(res.headers.location) === false) {
            reject(
              new Error(
                `Redirecting location "${res.headers.location}" isn't supported.`
              )
            )
            return
          }
          this._httpRequest(method, res.headers.location, option).then(
            resolve,
            reject
          )
          return
  ...
  private async _requestStream(
    ...
    if (res.statusCode >= 200 && res.statusCode <= 202) {
      ...
      if (res.statusCode) {
        throw new Error(
          `Bad status respond (${res.statusCode} ${res.statusMessage}).`
        )
      }
      throw res
    }
