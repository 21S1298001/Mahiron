...
import type { JoinParams, NotifyParams } from 'mahiron-server/src/Mirakurun/rpc'
import type { EventMessage } from 'mahiron-server/src/Mirakurun/Event'
import type { TunerDevice, Service, Status } from 'mahiron-server/types/api'
...
function idleStatusChecker(): boolean {
  ...
  const isActive = uiState.tuners.some(
    (tuner) =>
      tuner.isUsing === true && tuner.users.some((user) => user.priority !== -1)
  )
  if (isActive) {
    statusName = 'Active'
...
rpc.methods.set(
  'events',
  async (socket, { array }: NotifyParams<EventMessage>) => {
    ...
    for (const event of array) {
      if (event.resource === 'service') {
        ...
        uiState.tuners[
          uiState.tuners.findIndex((value) => value.index === tuner.index)
        ] = tuner
        uiStateEvents.emit('update:tuners')
      }
    }
    ...
    <Fabric style={{ margin: '16px' }}>
      <Stack tokens={{ childrenGap: '8 0' }}>
        ...
        >
          <img
            style={{ height: '96px' }}
            src={iconSrcMap[state.statusIconName]}
          />
