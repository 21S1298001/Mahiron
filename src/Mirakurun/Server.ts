  ...
  async init() {
    ...
    app.use((req: express.Request, res: express.Response, next) => {
      ...
      if (req.get('Referer') !== undefined) {
        if (
          !system.isPermittedHost(req.get('Referer'), serverConfig.hostname)
        ) {
          res.status(403).end()
          return
        }
    ...
    addresses.forEach((address) => {
      ...
      if (
        regexp.unixDomainSocket.test(address) === true ||
        regexp.windowsNamedPipe.test(address) === true
      ) {
