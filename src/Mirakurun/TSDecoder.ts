...
export default class TSDecoder extends stream.Writable {
  ...
  _write(chunk: Buffer, encoding: string, callback: () => void) {
    ...
    if (this._isNew === true && this._process) {
      ...
      this._timeout = setTimeout(() => {
        log.warn(
          'TSDecoder#%d process will force killed because no respond...',
          this._id
        )
        this._dead()
      }, 1500)
  ...
  private _spawn(): void {
    ...
    if (this._deadCount > 0) {
      ...
      log.warn(
        'TSDecoder#%d respawning because dead (count=%d)',
        this._id,
        this._deadCount
      )
    }
    ...
    log.info(
      'TSDecoder#%d process has spawned by command `%s` (pid=%d)',
      this._id,
      this._command,
      proc.pid
    )
  ...
  private _fallback(): void {
    ...
    log.warn(
      'TSDecoder#%d has been fallback into pass-through stream',
      this._id
    )
  }
