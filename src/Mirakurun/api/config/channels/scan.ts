...
function range(start: number, end: number): string[] {
  return Array.from({ length: end - start + 1 }, (v, index) =>
    (index + start).toString(10)
  )
}
...
export function generateScanConfig(option: ChannelScanOption): ScanConfig {
  ...
  Object.keys(option).forEach(
    (key) => option[key] === undefined && delete option[key]
  )
...
export function generateChannelItemForChannel(
  ...
  for (
    let servicesIndex = 1;
    servicesIndex < services.length;
    servicesIndex++
  ) {
    const service = services[servicesIndex]
    for (
      let nameIndex = 0;
      nameIndex < baseName.length && nameIndex < service.name.length;
      nameIndex++
    ) {
      ...
      if (
        nameIndex + 1 >= service.name.length &&
        service.name.length < matchIndex
      ) {
...
export function generateChannelItems(
  ...
  return [
    generateChannelItemForChannel(type, channel, services, setDisabledOnAdd),
  ]
}
...
export const put: Operation = async (req, res) => {
  ...
  const result: config.Channel[] = oldChannelItems.filter(
  ...
  for (let i = 0; i < chLength; i++) {
    ...
    res.write(
      `channel: "${channel}" (${i + 1}/${chLength}) [${Math.round(
        ((i + 1) / chLength) * 100
      )}%] ...\n`
    )

    if (!refresh) {
      const takeoverChannelItems = oldChannelItems.filter(
        (chItem) =>
          chItem.type === type &&
    ...
    services = services.filter(
      (service) => service.type === 1 || service.type === 173
    )
  ...
  res.write(
    `-> total ${newCount + takeoverCount}/${
      result.length
    } (${type}/Any) channels configured.\n\n`
  )
...
- BS "non" subchannel style scans and GR scans are basically the same. Note that if you scan the wrong channel range, the GR channel will be registered as BS and the BS channel will be registered as GR. This problem does not occur because CS scan uses a character string with \`CS\` added as a channel number prefix.`,
  ...
  parameters: [
    ...
    },
    {
      ...
      type: 'string',
      enum: [
        common.ChannelTypes.GR,
        common.ChannelTypes.BS,
        common.ChannelTypes.CS,
      ],
